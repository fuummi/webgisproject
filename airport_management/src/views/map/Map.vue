    ...
    <el-container>
      <el-aside>
        <el-tabs type="border-card" @tab-click="switchBtnClick">
          <el-backtop target=".el-tabs" style="left: 250px; bottom: 20px"></el-backtop>
          <el-tab-pane label="航班实时">
            ...
            <airlines-list :c-list="renderAirlinesList" @cardclick="cardClick"></airlines-list>
          </el-tab-pane>
      ...
      <el-main>
        ...
        <el-drawer title="热力图" :visible.sync="drawer">
          <heat-map-container class="heatmap-container" :cfeatures="features"></heat-map-container>
        </el-drawer>
      </el-main>
    </el-container>
  </div>
...
export default {
  ...
  methods: {
    ...
    async _getAirports() {
      ...
      result.data.forEach(item => {
        list[item['cn-name']] = [item.longitude_deg, item.latitude_deg];
      });
      this.data_geo = list;
    },
    /*
    ...
    */
    ...
    async _getAirlinesByTime() {
      const { data: result } = await this.$http.get('/api/airlines/findbytime', {
        params: {
          page: this.page,
        },
      });
      if (result.meta.status == 1) {
        this.renderAirlinesList = result.data.result.data;
    ...
    handleCurrentChange(page) {
      ...
      this._getAirlinesByTime().then(() => {
        ShowTrail.generateMigrationMap(this.viewer, this.renderAirlinesList, this.data_geo);
        this.queryId = '';
      });
    },
    // 航线查询
    getQueryList(list) {
      ...
      byFrom = byFrom ? 1 : 2;
      const arr = ShowTrail.generateMigrationMap(this.viewer, airlinesList, this.data_geo, byFrom);
      this.features = arr;
  ...
  async mounted() {
    ...
    viewer.homeButton.viewModel.command.beforeExecute.addEventListener(function (e) {
      ...
      //你要飞的位置
      viewer.camera.flyTo({
        destination: Cesium.Cartesian3.fromDegrees(114.703893, 38.287385, 10000000.0),
      });
    });

    viewer.camera.setView({
      destination: Cesium.Cartesian3.fromDegrees(114.703893, 38.287385, 10000000.0), // 设置位置
    ...
    ShowTrail.generateMigrationMap(viewer, this.renderAirlinesList, this.data_geo);
  },
  components: {
    AirlinesList,
    QueryAirlinesList,
