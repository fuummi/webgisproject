<template>
  <div class="container" @keydown.enter="queryBtnClick">
    <div class="query-container">
      <el-input size="mini" :placeholder="inputPlacehold" v-model="query" class="input-with-select">
        <el-select
          v-model="byFrom"
          slot="prepend"
          style="width: 65px"
        ...
        <el-button slot="append" icon="el-icon-search" @click="queryBtnClick"></el-button>
      </el-input>
    </div>
    <airlines-list :cList="airlinesList" :cTime="nowTime" @cardclick="cardClick"></airlines-list>
  </div>
</template>
  ...
  methods: {
    ...
    async _getAirlinesList() {
      ...
      if (result.meta.status == 1) {
        this.airlinesList = result.data.data;
